...
type Server struct {
	*command.Config
	gcc *client.GlusterClients
}

// Run starts the controller server
func Run(config *command.Config) {
	srv := csi.NewNonBlockingGRPCServer()
	srv.Start(config.Endpoint, identity.NewServer(config), NewServer(config), nil)
	srv.Wait()
}
...
func (cs *Server) CreateVolume(ctx context.Context, req *api.CreateVolumeRequest) (*api.CreateVolumeResponse, error) {
	...
	} else if len(glusterServers) == 0 {
		glog.Errorf("no hosts found for %s / %s", reqParams["glusterurl"], reqParams["glusteruser"])
		return nil, status.Errorf(codes.FailedPrecondition, "no hosts found for %s / %s", reqParams["glusterurl"], reqParams["glusteruser"])
	}
...
func (cs *Server) DeleteVolume(ctx context.Context, req *api.DeleteVolumeRequest) (*api.DeleteVolumeResponse, error) {
	...
	gc, err := cs.gcc.FindVolumeClient(volumeID, cs.Name)
...
func (cs *Server) ValidateVolumeCapabilities(ctx context.Context, req *api.ValidateVolumeCapabilitiesRequest) (*api.ValidateVolumeCapabilitiesResponse, error) {
	...
	_, err := cs.gcc.FindVolumeClient(volumeID, cs.Name)
	if err != nil {
...
func (cs *Server) ListVolumes(ctx context.Context, req *api.ListVolumesRequest) (*api.ListVolumesResponse, error) {
	...
	for server, users := range cs.gcc.Cache {
		for user, searchClient := range users {
			vols, listErr := searchClient.ListVolumes()
			if listErr != nil {
...
func (cs *Server) CreateSnapshot(ctx context.Context, req *api.CreateSnapshotRequest) (*api.CreateSnapshotResponse, error) {
	...
	gc, err := cs.gcc.FindVolumeClient(srcVol, cs.Name)
	if err != nil {
		code := codes.Internal
		if client.IsErrNotFound(err) {
			code = codes.AlreadyExists
...
func (cs *Server) DeleteSnapshot(ctx context.Context, req *api.DeleteSnapshotRequest) (*api.DeleteSnapshotResponse, error) {
	...
	gc, err := cs.gcc.FindVolumeClient(snapName, cs.Name)
	if err != nil && !client.IsErrNotFound(err) {
		glog.Error(err)
...
func (cs *Server) ListSnapshots(ctx context.Context, req *api.ListSnapshotsRequest) (*api.ListSnapshotsResponse, error) {
	...
	for server, users := range cs.gcc.Cache {
		for user, gc := range users {
			snaps, listErr := gc.ListSnapshots(snapName, srcVol)
			if listErr != nil {
				glog.V(1).Infof("failed to list snapshots for %s / %s: %v", server, user, err)
				continue
			}
