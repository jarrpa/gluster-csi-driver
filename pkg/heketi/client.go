...
// NewClient returns a new heketiClient
func NewClient(config *command.Config) (cli.GlusterClient, error) {
	var err error

	if len(config.RestUser) == 0 {
		config.RestUser = "admin"
	}
...
func (gc heketiClient) setClient(client heketiClient) (heketiClient, error) {
	if gc.client == nil {
...
func (gc heketiClient) GetControllerCapabilities() []csi.ControllerServiceCapability_RPC_Type {
